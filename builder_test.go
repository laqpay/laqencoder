...
import (
	...
	_ "github.com/laqpay/laqpay/src/coin" // needed to verify test output
)
...
func TestBuildLaqpaySignedBlock(t *testing.T) {
	importPath := "github.com/laqpay/laqpay/src/coin"
	...
	filename := filepath.Join(fullPath, "signed_block_laqencoder_xxxyyy.go")
...
func TestBuildFails(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			testBuildCodeFails(t, tc.name, fmt.Sprintf("./%s_laqencoder_test.go", ToSnakeCase(tc.name)))
		})
	}
}
