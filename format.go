...
func buildTest(typeName, typePackageName, packageName string, hasMap, exported bool) string {
	...
	return fmt.Sprintf(`// Code generated by github.com/laqpay/laqencoder. DO NOT EDIT.
...
func testLaqencoder%[1]s(t *testing.T, obj *%[2]s) {
...
func TestLaqencoder%[1]s(t *testing.T) {
...
func testLaqencoder%[1]sDecodeErrors(t *testing.T, k int, tag string, obj *%[2]s) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestLaqencoder%[1]sDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testLaqencoder%[1]sDecodeErrors(t, i, "empty", emptyObj)
		testLaqencoder%[1]sDecodeErrors(t, i, "full", fullObj)
	}
}
