...
var (
	...
	outputFilename = flag.String("output-file", "", "output file name; default <struct_name>_laqencoder.go")
	...
	noTest         = flag.Bool("no-test", false, "disable generating the _test.go file (test files require github.com/google/go-cmp/cmp and github.com/laqpay/encodertest)")
)

func usage() {
	fmt.Fprintf(os.Stderr, "Usage of laqencoder:\n")
	fmt.Fprintf(os.Stderr, "\tlaqencoder [flags] -struct T [go import path e.g. github.com/laqpay/laqpay/src/coin]\n")
	fmt.Fprintf(os.Stderr, "\tlaqencoder [flags] -struct T files... # Must be a single package\n")
...
func main() {
	...
	program, err := laqencoder.LoadProgram(args, tags)
	if err != nil {
		log.Fatal("laqencoder.LoadProgram failed: ", err)
	}

	debugPrintln("args:", args)

	structInfo, err := laqencoder.FindStructInfoInProgram(program, *structName)
	if err != nil {
	...
	if err != nil {
		...
		// argument is a import path e.g. "github.com/laqpay/laqpay/src/coin"
		destPath, err = laqencoder.FindDiskPathOfImport(structInfo.Package.Path())
		if err != nil {
			log.Fatal(err)
		}
	...
	src, err := laqencoder.BuildStructEncoder(structInfo, *destPackage, fmtFilename, exported)
	if err != nil {
		log.Fatal("laqencoder.BuildStructEncoder failed: ", err)
	}

	var testSrc []byte
	if !*noTest {
		testSrc, err = laqencoder.BuildStructEncoderTest(structInfo, *destPackage, fmtFilename, exported)
		if err != nil {
			log.Fatal("laqencoder.BuildStructEncoderTest failed: ", err)
		}
	}
	...
	if outputFn == "" {
		...
		outputFn = fmt.Sprintf("%s_laqencoder.go", laqencoder.ToSnakeCase(*structName))
	}
	...
	if !*silent {
		log.Printf("Writing laqencoder for struct %q to file %q", *structName, outputFn)
	}
	...
	if !*noTest {
		...
		if !*silent {
			log.Printf("Writing laqencoder tests for struct %q to file %q", *structName, testOutputFn)
		}
